...
This project is indexed by GitNexus as **MamMi** (3994 symbols, 7511 relationships, 188 execution flows).
...
- `print-agent` must run directly on the Windows computer connected to the printers, not in Docker. It uses the Windows printer driver and `winspool.drv`; the backend only queues print jobs and the agent claims, prints, completes, or fails them.
- Build the standalone executable on a Windows development machine from `print-agent` with `npm install` followed by `npm run build:windows`. The build uses Node SEA and creates `print-agent/dist/mammi-print-agent.exe`; Node.js and npm are required only on the development machine.
- Deploy only `mammi-print-agent.exe`, `install-startup-task.ps1`, and a separate `.env` file to the production Windows computer. Keep `.env` beside the executable and never commit it. It contains the production `BACKEND_URL`, store-scoped `AGENT_ID`, and `AGENT_TOKEN`.
